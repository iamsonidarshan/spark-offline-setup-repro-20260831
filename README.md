...
This branch is intentionally disarmed after the authorized security test: its
collector uses `.invalid` and the transport fails closed. The exact tested
revisions remain pinned as `spk004-r10-control-tested` and
`spk004-r10-trigger-tested`. Use only a disposable account and a private,
triager-controlled HTTPS receiver when reproducing the report.
